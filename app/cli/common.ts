 ...
 */
export const generate_manual_continue = async (execute: () => Promise<any>, message: string = '是否继续？') => {
...
export function generateData(rootPath: string, config = { baseUrl: 'https://iptv-assets.oss-cn-beijing.aliyuncs.com/', basePath: 'dist/' }): Array<Group> {
  ...
  groupFiles.forEach(file => {
    ...
    for (let i = 0; i < tableData.length; i++) {
      ...
      let currentChannels = group['group_data'].find(item => item.name === groupName);
      if (!currentChannels) {
        currentChannels = {
          name: groupName,
          channels: [],
      ...
      let channelMd = '';
      try {
        channelMd = fs.readFileSync(path.resolve(rootPath, 'channel', groupChannel + '.md'), 'utf-8');
      } catch (err) {
...
export function generateMarkdownWithGroup(group: ChannelGroup) {
  ...
  group.channels?.forEach(channel => {
    md += `| ${channel.name} | ${channel.alias || ''} | ${channel.logo || ''} |\n`;
  });
  md = md.replace(/\n$/, ''); // 去除最后的换行符
  return md;
}
